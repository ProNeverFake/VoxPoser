...
class Controller:
    ...
    def _calculate_ee_rot(self, pushing_dir):
        ...
        desired_dir = pushing_dir + np.array([0, 0, -np.linalg.norm(pushing_dir)]) # offset to the table
        desired_dir = normalize_vector(desired_dir)
        left = np.cross(pushing_dir, desired_dir) # it is the left direction of the pushing direction
        left = normalize_vector(left)
        up = np.array(desired_dir, dtype=np.float32) # it is not exactly up, but slanted towards the table
        up = normalize_vector(up)
        forward = np.cross(left, up) # it is not exactly forward, but slanted backwards towards the table
        forward = normalize_vector(forward)
        rotmat = np.eye(3).astype(np.float32)
        # * up is the z, this means the ee is still tending to the pushing direction
        rotmat[:3, 0] = forward
        rotmat[:3, 1] = left
        rotmat[:3, 2] = up
    ...
    def calculate_cost(self, obs_sequences, controls_sequences, target_xyz):
        """
        Calculate the cost of the generated control sequence (of each batch)
        T Time
        B Batch size
        N Number of points in each observation
        inputs:
        ...
        # calculate the distance cost of each batch
